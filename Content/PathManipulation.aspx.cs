        ...
        {	
        	...
        	//
        		...
        		if(filename != null)
        		{
                    //try
                    //{
                      //  File.AppendAllText(@"c:\paths.txt", DateTime.Now + " " + filename + "\r\n"); // debug only //
        	...
        	/*
                if (!string.IsNullOrEmpty(Request.QueryString["filename"]) && this.Equals(this))
                {
                    try
                    {
                        ResponseFile(Request, Response, Request.QueryString["filename"], MapPath("~/Downloads/" + Request.QueryString["filename"]), 100);
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine(ex.Message);
                        lblStatus.Text = "File not found: " + Request.QueryString["filename"];
                    }
                }
             */
        }
